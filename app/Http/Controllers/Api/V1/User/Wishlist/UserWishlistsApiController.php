...
use App\Http\Requests\Website\User\Wishlists\AddtowishlistApiRequest;
use App\Http\Requests\Website\User\Wishlists\DeletefromwishlistApiRequest;

class UserWishlistsApiController extends Controller
{
    public function addtowishlist(AddtowishlistApiRequest $request)
    {
    	$prod_id  = $request->product_id;
        $quantity = 1; //$request->quantity;
    	
        if(Cookie::get('user_wishlist')){
        	//return 'get';
            $cookie_data = stripslashes(Cookie::get('user_wishlist'));
        ...
        if(in_array($prod_id_is_there, $item_id_list))
        {
            ...
            foreach($cart_data as $keys => $values)
            {
                if($cart_data[$keys]["item_id"] == $prod_id)
                {
                    ...
                    $response->withCookie('user_wishlist', $item_data, $minutes);
                    return $response; 
                }
            }
        }
    ...
    public function updatetowishlist(MakeOrderApiRequest $request)
    {
        $prod_id  = $request->product_id;
        $quantity = 1;//$request->quantity;
    ...
    public function deletefromwishlist(DeletefromwishlistApiRequest $request)
    {
    ...
    {
    	$cookie = Cookie::forget('user_wishlist');
        return response()->json(['status'=>'Your Wishlist is Cleared'])->withCookie($cookie);
