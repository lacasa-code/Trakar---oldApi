...
class SpecificFrontProductsApiResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        return[
           ...
           'time_created'         => Carbon::createFromFormat('Y-m-d H:i:s', $this->created_at)->format('Y-m-d H:i:s'),
         // 'in_cart'  => ($this->user == null ? 0 : $this->user->revise_cart($this->id)),
         // 'in_wishlist' => ($this->user == null ? 0 : $this->user->revise_wishlist($this->id)),
         // 'in_favourites' => ($this->user == null ? 0 : $this->user->revise_favourites($this->id)),
    
             'in_cart'             => $this->in_cart,
             'in_wishlist'         => $this->in_wishlist,
             'in_favourites'       => $this->in_favourites,
           // 'in_wishlistt' => $this->user->revise_wishlist($this->id),

        // new
      'can_add_question' => $this->user == null ? 0 : (Productquestion::where('user_id', $this->user->id)
                                    ->where('product_id', $this->id)->count() > 0 ? 0 : 1),
        'can_add_review' => $this->user == null ? 0 : (Productreview::where('user_id', $this->user->id)
                                    ->where('product_id', $this->id)->count() > 0 ? 0 : 1),
        ];
    }
}
