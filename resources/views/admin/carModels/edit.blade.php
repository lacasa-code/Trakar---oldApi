    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.car-models.update", [$carModel->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carmade') ? 'is-invalid' : '' }}" name="carmade_id" id="carmade_id" required>
                    @foreach($carmades as $id => $carmade)
                        <option value="{{ $id }}" {{ (old('carmade_id') ? old('carmade_id') : $carModel->carmade->id ?? '') == $id ? 'selected' : '' }}>{{ $carmade }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('carmodel') ? 'is-invalid' : '' }}" type="text" name="carmodel" id="carmodel" value="{{ old('carmodel', $carModel->carmodel) }}" required>
