...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.addVendor.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.add-vendors.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('userid') ? 'is-invalid' : '' }}" name="userid_id" id="userid_id" required>
                    @foreach($userids as $id => $userid)
                        <option value="{{ $id }}" {{ old('userid_id') == $id ? 'selected' : '' }}>{{ $userid }}</option>
                    @endforeach
                </select>
